...
if __name__ == "__main__":
    ...
    # print("Load model.") 
    # # load model（加载模型）
    # unet = Unet(model_path = "./logs/logs_2023_09_27_19_42_03 - model12 -/model_best/best_epoch_weights.pth")
    # print("Load model done.")

    # predicted result dir（预测结果存放路径，miou等计算结果也存在这里）
    model_id        = "voted"
    miou_out_path   = "./miou_out/" + model_id + ' - ' + time.strftime('%Y_%m_%d %H.%M.%S - predict info',time.localtime(time.time()))
    ...
    if miou_mode == 0 or miou_mode == 1:
        ...
        for image_id in tqdm(image_ids):
            ...
            r_image, model_id, models_id, models_weights = Unet_vote().get_result_colors(image_bk)
    ...
    if miou_mode == 0 or miou_mode == 2:
        ...
        # print(PA_Recall)
        # print(Precision)
        show_results(miou_out_path, hist, IoUs, PA_Recall, Precision, name_classes,model_id)
    ...
    with open (f_name,'w') as f:
        ...
        f.write(str(models_id))
        f.write("\n")
        f.write(str(models_weights))
        f.write("\n")
