...
class unetUp(nn.Module):
    def __init__(self, in_size, out_size, upconv_size):
...
class Unet(nn.Module):
    def __init__(self, num_classes = 21, pretrained = False, backbone = 'vgg'):
        ...
        self.up_concat4 = unetUp(in_filters[3], out_filters[3], upConv_filters[3])
        # 128,128,256
        # 一个上采样操作单元，若vgg：unetUP(768,256)
        self.up_concat3 = unetUp(in_filters[2], out_filters[2], upConv_filters[2])
        # 256,256,128
        # 一个上采样操作单元，若vgg：unetUP(384,128)
        self.up_concat2 = unetUp(in_filters[1], out_filters[1], upConv_filters[1])
        # 512,512,64
        # 一个上采样操作单元，若vgg：unetUP(192,64)
        self.up_concat1 = unetUp(in_filters[0], out_filters[0], upConv_filters[0])

        if backbone == 'resnet50':
            self.up_conv = nn.Sequential(
                # nn.ConvTranspose2d(scale_factor = 2), 
                # 线性插值改为转置卷积
    ...
    def forward(self, inputs):
        ...
        # print("up4：",up4.size())
        up3 = self.up_concat3(feat3, up4)   # 若使用nn.ConvTranspose2d,则RuntimeError
