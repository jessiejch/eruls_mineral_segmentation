...
if __name__ == '__main__':
    # 处理完后jpg图像存放位置（可能需要修改）
    # jpgs_path   = "./datasets/JPEGImages/"
    jpgs_path   = "./img/512size_test_img/"
    # 处理完后png label存放位置（可能需要修改）
    # pngs_path   = "./datasets/SegmentationClass/"
    pngs_path   = "./img/512size_test_label/"
    ...
    # 将类别修改如下：
    classes     = ["_background_","pyrite","galena","sphalerite","chalcopyrite","bornite","magnetite","pyrrhotite","gangue"]
    # 双引号内路径是原图像和标注的json文件存放位置（可能需要修改）
    count = os.listdir("./datasets/test_data/512size_img_json") 
    for i in range(0, len(count)):
        # 双引号内路径是原图像和标注的json文件存放位置（可能需要修改）
        path = os.path.join("./datasets/test_data/512size_img_json", count[i])
